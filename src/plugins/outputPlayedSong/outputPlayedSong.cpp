...
static std::wstring songNameEx = L"";
static std::wstring songArtist;
static std::wstring songArtistEx = L"";
static std::wstring prefix = L"";
static bool prefSet = false;

typedef unsigned short word;

static std::wstring AAnThe[] = {
	L"A",
	L"AN",
	L"THE"
};

static std::wstring OtherLowercaseWords[] = {
	L"IS",
	L"TO",
	L"FOR",
	L"AND",
...
void ApplyWordLowercasing(std::wstring & ShoutString, int Start, int LetterCount, bool FirstLetterIsToBeCapital)
{
	for (int i = Start; i <= Start + LetterCount; i++)
	{
		if (i == Start && FirstLetterIsToBeCapital)
		{
			continue;
		}

		for (int j = 0; j < 9 * 74; j++)
		{
			if ((int)(ShoutString[i]) == Uppercase[j])
			{
				ShoutString[i] = Lowercase[j];
				break;
			}
		}
	}
...
void ProcessUnicodeArtistName(std::wstring & ArtistName)
{
	bool bDuringWord = false;
	int LastWordStart = -1;

	//fill special/whitespace chars at the end with null

	for (int i = ArtistName.length() - 1; (i >= 0); i--)
	{
		if (ArtistName[i] != 0x0020 && ArtistName[i] != 0x0000)
		{
			break;
		}
		ArtistName[i] = 0x0000;
	...
	{
		if (ArtistName[i] == 0x0020 || (i == ArtistName.length() - 1))
		{
			if (bDuringWord) //the word ended
			{
				bDuringWord = false;
				ApplyWordLowercasing(ArtistName, LastWordStart, i - LastWordStart + ((i == ArtistName.length() - 1) ? (0) : (-1)), true);
			}
		}
		else
		{
...
void ProcessUnicodeSongName(std::wstring & SongName)
{
	int wordCounter = -1;
	bool bDuringWord = false;
	int LastWordStart = -1;
	...
	for (int i = SongName.length() - 2; (i >= 0); i--)
	{
		if (SongName[i] != 0x0020 && SongName[i] != 0x0000)
		{
			break;
		}
		SongName[i] = 0x0000;
	...
	for (unsigned int i = 0; i < SongName.length(); i++)
	{
		if (SongName[i] == 0x0020 || (i == SongName.length() - 1))
		{
			if (bDuringWord) //the word ended
			{
				wordCounter++;
				bool bAAnTheFound = false;
				for (int j = 0; j < 3; j++)
				{
					if (AAnThe[j].length() == i - LastWordStart + 1 + ((i == SongName.length() - 1) ? (0) : (-1)))
					{
						if (wcsncmp(AAnThe[j].c_str(), SongName.substr(LastWordStart, AAnThe[j].length()).c_str(), AAnThe[j].length()) == 0)
						{
							if (wordCounter == 0)
							{
								wordCounter--; //with "a", "an" or "the" as first word, the following word should be treated as first (recursively) eg. "the a Banana" 
								bCapitalize = (i == SongName.length() - 1); //if it is the only word in song name (excluding a, an and the) (it also produces "the A" and "a An")
							}
							else
							{
								bCapitalize = false;
				...
				if (!bAAnTheFound)
				{
					...
					{
						if (OtherLowercaseWords[j].length()== i - LastWordStart + 1 + ((i == SongName.length() - 1) ? (0) : (-1)))
						{
							if (wcsncmp(OtherLowercaseWords[j].c_str(), SongName.substr(LastWordStart, OtherLowercaseWords[j].length()).c_str(), OtherLowercaseWords[j].length()) == 0)
							{
								bCapitalize = wordCounter == 0 || (i == SongName.length() - 1); //only first or last word
								break;
							}
						}
					}
				...
				bDuringWord = false;
				ApplyWordLowercasing(SongName, LastWordStart, i - LastWordStart + ((i == SongName.length() - 1) ? (0) : (-1)), bCapitalize);
			}
		}
		else
		{
			if (!bDuringWord)
			{
				bDuringWord = true;
				LastWordStart = i;
				bCapitalize = true;//capitalize words by default
			}
		}
	}
}
...
std::wstring GetPrefix()
{
	...
	std::ifstream aif("prefixansi.txt", std::ios::binary);
	if (aif.is_open())
	{
		std::string sline;
		std::getline(aif, sline);
		aif.close();
		std::wstring w(sline.begin(), sline.end());
		if (w != L"")
		{
			prefix = w;
			return w;
	...
	std::wifstream wif("prefixutf16.txt", std::ios::binary);
	if (wif.is_open())
	{
		std::wstring wline;
		// cut BOM of utf16
...
std::wstring ProcessFullSongName(std::wstring & Song, std::wstring & Artist, bool AddPrefix = true)
{
	std::wstring temp = L"";
	if (AddPrefix)
	{
	...
	ProcessUnicodeSongName(Song);
	ProcessUnicodeArtistName(Artist);
	temp += Artist + L"-" + Song;
	return temp;
}

DWORD WINAPI ProcessSongFileWrite(LPVOID l);
...
void __cdecl setScreenElementPropsOverride(GH3::QbStruct* args)
{
	...
	if((pluginState & 3) == 3)
	{
		...
		if ((songNameEx != songName || songArtistEx != songArtist))//dont apply changes if the song is the same
		{
			songArtistEx = songArtist;
			songNameEx = songName;
			CreateThread(NULL, 0, ProcessSongFileWrite,	NULL, 0, NULL);
		}
	}
}
...
DWORD WINAPI ProcessSongFileWrite(LPVOID l)
{
	std::wstring sn = songNameEx;//need temporary value
	std::wstring sa = songArtistEx;//need temporary value
	GetPrefix();
	ProcessFullSongName(sn, sa);//change only, not using ret value
